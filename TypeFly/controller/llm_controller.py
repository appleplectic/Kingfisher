...
class LLMController():
    def __init__(self, robot_type, use_http=False, message_queue: Optional[queue.Queue]=None, spark_url: str=None, spark_interface: str=None):
        ...
        match robot_type:
            ...
            case RobotType.SPARK:
                print_t("[C] Start Spark drone...")
                if spark_url is None or spark_interface is None:
                    raise ValueError("A URL and an interface must be given using the command line parameters --url and --interface.")
                self.drone: RobotWrapper = SparkWrapper(spark_url, spark_interface)
            case _:
